...
void IpNegotiator::initialize(connecttool::domain::PeerId localPeerId, uint32_t baseIP,
                              uint32_t subnetMask) {
  localPeerId_ = localPeerId;
  baseIP_ = baseIP;
  subnetMask_ = subnetMask;
  localNodeId_ = NodeIdentity::generate(localPeerId);
...
void IpNegotiator::checkTimeout() {
  ...
  bool canClaim = true;
  std::vector<connecttool::domain::PeerId> nodesToForceRelease;
  ...
  for (const auto &conflict : conflicts) {
    ...
    if (NodeIdentity::hasPriority(localNodeId_, conflict.nodeId)) {
      nodesToForceRelease.push_back(conflict.senderPeerId);
    } else {
      canClaim = false;
      break;
    }
  ...
  if (canClaim) {
    for (auto peerId : nodesToForceRelease) {
      sendForcedRelease(candidateIP_, peerId);
    }
...
void IpNegotiator::handleProbeRequest(const ProbeRequestPayload &request,
  ...
  if (shouldRespond && sendCallback_) {
    ...
    sendCallback_(VpnMessageType::PROBE_RESPONSE, reinterpret_cast<const uint8_t *>(&response),
                  sizeof(response), senderPeerId, true);
...
void IpNegotiator::handleProbeResponse(const ProbeResponsePayload &response,
  ...
  info.senderPeerId = senderPeerId;
...
void IpNegotiator::handleAddressAnnounce(const AddressAnnouncePayload &announce,
  ...
  if (announcedIP == localIP_ && state_ == NegotiationState::STABLE) {
    ...
    sendForcedRelease(announcedIP, peerId);
    return;
  }
...
void IpNegotiator::sendAddressAnnounceTo(connecttool::domain::PeerId targetPeerId) {
  ...
  sendCallback_(VpnMessageType::ADDRESS_ANNOUNCE, reinterpret_cast<const uint8_t *>(&payload),
                sizeof(payload), targetPeerId, true);
}

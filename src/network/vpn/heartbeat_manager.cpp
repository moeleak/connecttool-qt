...
void HeartbeatManager::registerNode(const NodeID &nodeId, connecttool::domain::PeerId peerId,
                                    uint32_t ipAddress,
  ...
  if (it != ipToNodeId_.end() && it->second != senderNodeId) {
    ...
    if (NodeIdentity::hasPriority(it->second, senderNodeId)) {
      ...
      if (nodeIt != nodeTable_.end()) {
        return ConflictResolution{.loserPeerId = nodeIt->second.peerId,
                                  .winnerNodeId = it->second};
      }
    } else {
      auto nodeIt = nodeTable_.find(it->second);
      if (nodeIt != nodeTable_.end()) {
        const auto loser = nodeIt->second.peerId;
        it->second = senderNodeId;
        return ConflictResolution{.loserPeerId = loser, .winnerNodeId = senderNodeId};
      }
    }
  }
  return std::nullopt;

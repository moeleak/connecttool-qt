...
public:
  ...
  std::unique_ptr<connecttool::network::TcpTunnelServer> *&getServer() { return server_; }
  ...
  void setMessageHandlerDependencies(boost::asio::io_context &io_context,
                                     std::unique_ptr<connecttool::network::TcpTunnelServer> &server,
                                     int &localPort,
                                     int &localBindPort);
  ...
  std::string registerTcpClient(std::shared_ptr<boost::asio::ip::tcp::socket> socket,
